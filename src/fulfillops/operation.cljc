(ns fulfillops.operation
  "OperationActor — one fulfilment request = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor (FulfillmentAdvisor)
  is sealed into a single node (:advise); its proposal is ALWAYS routed
  through the FulfillmentGovernor (:govern) and the rollout phase gate
  ...
  One graph run = one fulfilment request (intake -> advise -> govern ->
  ...
  campaign-operations reviewer. The approver resumes with
  `{:approval {:status :approved :by \"..\"}}` (or :rejected).

  Because `:disclose-failure` always escalates (governor AND phase,
  independently), the ONLY path by which this fleet records that a creator
  cannot deliver runs through this interrupt. A named human is
  structurally unavoidable, which is the point: that statement belongs to
  the creator, and an actor making it on their behalf would be putting
  words in someone's mouth about the thing backers care most about."
  (:require [fulfillops.advisor :as advisor]
